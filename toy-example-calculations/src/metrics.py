...
def epc(rec_list: List[Dict], 
    ...
    Higher = more novel relevant items at top positions. Range [0, 1]."""
    score = 0.0
    normalizer = 0.0
...
def cr_gini(rec_list: List[Dict], archives: List[str]) -> float:
    """Collection Representation via 1 - Gini coefficient.
    Higher = more equal distribution across provider groups. Range [0, 1]."""
...
def ca_eps(rec_list: List[Dict],
    ...
    Higher = recommendations better match user's research context. Range [0, 1]."""
    score = 0.0
    normalizer = 0.0
...
def mwr(rec_list: List[Dict], use_relevance: bool = True) -> float:
    ...
    Higher = more relevant items with better metadata quality. Range [0, 1]."""
    if len(rec_list) == 0:
        return 0.0
...
def dri_ils(rec_list: List[Dict]) -> float:
    ...
    Higher = items more thematically related, aiding comparative analysis.
    Range [0, 1]. sim(i,j) = 1 - |sem_i - sem_j| in this instantiation."""
    n = len(rec_list)
    if n <= 1:
        return 0.0
...
def csva(metrics: Dict[str, float],
    ...
    Higher = better alignment with the given stakeholder weight profile."""
    if thresholds:
